...
import { obtenerProximasCitas, solicitarReasignacion, obtenerMisCancelaciones, obtenerMisReasignaciones } from '../servicios/api';
import { Cita } from '../tipos';

// Motivos de reasignacion disponibles
const MOTIVOS = [
  'Non me ven ben a hora',
  'Cambio de plans',
  'Prefiro outro dia',
  'Outro motivo',
];
...
export default function ReasignarCitaScreen({ navigation }: any) {
  ...
  async function enviarSolicitude() {
    if (citaSeleccionada === null || motivoSeleccionado === null) {
      Alert.alert('Atencion', 'Selecciona unha cita e un motivo de reasignacion.');
      return;
    }
    ...
    try {
      setEnviando(true);
      const resposta = await solicitarReasignacion(
        cita.fecha,
        cita.hora,
        cita.servicio,
        motivo,
      ...
      if (resposta.exito) {
        navigation.replace('SolicitudeReasignacionEnviada', { cita, motivo });
      } else {
    ...
    <View style={estilos.contenedor}>
      ...
      <View style={estilos.cabeceira}>
        ...
        <View style={estilos.cabeceiraTextos}>
          <Text style={estilos.tituloCabeceira}>Reasignar Cita</Text>
          <Text style={estilos.subtituloCabeceira}>Solicita cambiar a data dunha cita</Text>
        </View>
      </View>
      ...
      <ScrollView style={estilos.scroll} contentContainerStyle={estilos.scrollContido}>
        ...
        <View style={estilos.bannerAdvertencia}>
          ...
          <Text style={estilos.textoBanner}>
            A nova data sera asignada polo xerente.
          </Text>
        </View>

        {/* Seccion: Citas reasignables */}
        <Text style={estilos.tituloSeccion}>Selecciona a cita a reasignar</Text>

        {cargando ? (
          <ActivityIndicator size="large" color={Colores.primarioClaro} style={{ marginVertical: 24 }} />
        ) : citas.length === 0 ? (
          <View style={estilos.tarxetaVacia}>
            <Text style={estilos.textoVacio}>Non tes citas futuras para reasignar.</Text>
          </View>
        ) : (
        ...
        {/* Seccion: Motivo da reasignacion */}
        <Text style={[estilos.tituloSeccion, { marginTop: 24 }]}>Motivo da reasignacion</Text>
        ...
        <Text style={[estilos.tituloSeccion, { marginTop: 24 }]}>Comentario adicional</Text>
        <TextInput
          style={estilos.inputComentario}
          placeholder="Indica preferencias de horario ou calquera detalle..."
        ...
        <View style={estilos.caixaInfo}>
          ...
          <View style={estilos.caixaInfoPaso}>
            ...
            <Text style={estilos.caixaInfoTexto}>A solicitude e enviada ao xerente</Text>
          </View>
          <View style={estilos.caixaInfoPaso}>
            <Text style={estilos.caixaInfoNumero}>2.</Text>
            <Text style={estilos.caixaInfoTexto}>O xerente buscara un novo horario</Text>
          </View>
          <View style={estilos.caixaInfoPaso}>
            <Text style={estilos.caixaInfoNumero}>3.</Text>
            <Text style={estilos.caixaInfoTexto}>Recibiras unha notificacion co novo horario</Text>
          </View>
        </View>
        ...
        >
          {enviando ? (
            ...
            <Text style={estilos.botonEnviarTexto}>Solicitar reasignacion</Text>
          )}
        </TouchableOpacity>
      </ScrollView>
    </View>
